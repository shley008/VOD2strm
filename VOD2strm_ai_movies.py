...
def build_movie_proxy_url(proxy_host: str, movie: dict) -> str:
    """
    For movies write Dispatcharr's actual VOD streaming proxy URL to the STRM file.

    Target format:
      https://dispatcharr.falconridge.it/proxy/vod/movie/<movie_uuid>/<session_id>?stream_id=<provider_stream_id>

    This replaces the old .m3u8 URL:
      /proxy/vod/movie/<m3u_account_id>/<movie_uuid>/stream.m3u8
    """
    proxy_host = proxy_host.rstrip("/")
    vod_uuid = movie.get("uuid") or ""
    stream_id = get_movie_stream_id(movie)

    if vod_uuid and stream_id not in (None, ""):
        session_id = build_vod_session_id(str(vod_uuid), stream_id)
        return f"{proxy_host}/proxy/vod/movie/{vod_uuid}/{session_id}?stream_id={stream_id}"

    # Fallback only if the provider stream_id is not present in the API payload.
    # This keeps the script from failing outright while making the missing field visible.
    log(
        f"WARNING: Could not find provider stream_id for movie "
        f"'{movie.get('name') or movie.get('id')}'. Writing session VOD URL without stream_id."
    )
    if vod_uuid:
        session_id = build_vod_session_id(str(vod_uuid), movie.get("id") or "")
        return f"{proxy_host}/proxy/vod/movie/{vod_uuid}/{session_id}"

    raise ValueError(f"Movie '{movie.get('name') or movie.get('id')}' has no uuid; cannot build Dispatcharr VOD URL")
